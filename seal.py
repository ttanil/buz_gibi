...
class Seal(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.seal_x_pos=150                                                                                             #fokun x konumu
        self.seal_y_pos=560                                                                                             #fokun y konumu
        ...
        self.definningSealLocation(140,155,535)
    ...
    def update(self):
        ...
        elif self.status=="playing" and self.counter%2==0 and (self.status !="game_over_by_treasure" or self.status !="game_over"):
    ...
    def beginning(self):
        self.rect.midbottom = (self.seal_x_pos-10,self.seal_y_pos+50)
        self.seal_x_pos += 1
    ...
    def waitingForPlaying(self):
        ...
        if self.counter==0 and self.chilling==False and self.seal_diving_control==False:
            self.chilling=True
            self.definningSealLocation(random.randint(70,150), 300, random.randint(410,540))
        if self.chilling==True and self.counter<900 and self.seal_diving_control==False:
            if self.counter%3==0:
                self.rect.midbottom = (self.seal_x_pos - 10, self.seal_y_pos + 50)
                self.seal_x_pos += 1
    ...
    def positionCorrectionWithFace(self):
        if self.face == "right":
            ...
            self.rect.midbottom = (self.seal_x_pos +10, self.seal_y_pos + 45)

    def definningSealLocation(self,posx1,posx2,posy):
        self.seal_x_pos,self.seal_y_pos =random.randint(posx1,posx2),posy                                               #fok için ekranda yer beliler
        random_control=True
        while random_control:
            ...
            for konum in self.ice_on_sea_number:
                if konum[0] <= self.seal_x_pos <= konum[1] and konum[2] <= self.seal_y_pos <= konum[3]:
                    self.seal_x_pos,self.seal_y_pos =random.randint(posx1,posx2),posy
                    break
                else:
                    toplam+=1
            if toplam==len(self.ice_on_sea_number):
                random_control=False
        self.seal_up_to_the_water_point=self.seal_x_pos
        if self.seal_x_pos<640:                                                                                         #ekrandaki yerine göre fokun yüzünün döneceği tarafı belirler
            self.face = "right"
            self.image = self.image_seal_in_the_sea1
        else:
            self.face = "left"
    ...
    def backToTheWater(self):                                                                                           #bu fonksiyon fok'un yüzmesinin bitiminde
        for konum in self.ice_on_sea_number:                                                                            #önünde buz olup olmadığını kontrol eder yoksa
            if konum[0] <= self.seal_x_pos <= konum[1] and konum[2] <= self.seal_y_pos <= konum[3]:                     #dalış yaptır.(ardından tekrar yüzer)
    ...
    def sealSeaMovements(self):
        self.penguin_last_posy_list.clear()                                                                             #fokun penguenin olduğu yere gitme durumunu kuvvetlendirmek için
        for i in range(11):                                                                                             #y konumunun belirlendiği liste içine 11 tane penguenin son y pozisyon
            self.penguin_last_posy_list.append(self.penguin_posy)                                                       #bilgisini ekler
        if self.seal_sea_movements_control==False:
            total_list=breaking.makePosyList()+self.penguin_last_posy_list
            self.definningSealLocation(100, 1000, random.choice(total_list))           #self.definningSealLocation(100, 300, 390)              # self.definningSealLocation(50,400,self.posy-60,self.posy+60)
        ...
        if self.seal_aim_number != 5 and self.seal_sea_movements_control == True:
            ...
            if self.seal_sea_movements_counter>=800:
                self.sealDiving("dive")
                if self.seal_diving_control == False:
                    self.seal_sea_movements_control = False
                    self.direction = "none"
    ...
    def downFromIce(self):                                                                                              #fok buzların üstünde yürürken
        ...
        for konum in range(0,len(self.ice_on_sea_number)):                                                              #olmadığını kontrol eder, buz olmaması
            if (self.ice_on_sea_number[konum][0] <= self.seal_x_pos <= self.ice_on_sea_number[konum][1] and             #durumunda dalış durumuna geçirir
                    self.ice_on_sea_number[konum][2] <= self.seal_y_pos <= self.ice_on_sea_number[konum][3]):           #(tekrar yüzmeye dönmesini sağlar)
                break
            else:
                toplam+=1
        if toplam==len(self.ice_on_sea_number):
    ...
    def sealStepsOnIce(self):
        self.rect.midbottom = (self.seal_x_pos - 10, self.seal_y_pos+3)
